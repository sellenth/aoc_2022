...
let DP_table: number[] = new Array( (1 << num_worth_opening) * valves.length * 27 * 2 ).fill(-1)

function solve(curr_valve_idx: number, opened: number, time: number, player: number): number {
  if (time == 0)
    if (player == 1) {
      return 0
    } else {
      return solve(str_to_idx['AA'], opened, 26, 1)
    }


  let key = (opened * valves.length * 31 * 2) +
            (curr_valve_idx * 31 * 2) +
            (time * 2) +
            (player)
...
console.log(solve(str_to_idx['AA'], 0, 26, 0 ))